...
const fetchdata = async function () {
  ...
  //       let card = {
  //         product_name: data.cpus[i].productName,
  //         resolution: data.cpus[i].resolution,
  //         display_size: data.cpus[i].displaySize,
  //         panel_type: data.cpus[i].panelType,
  //         refresh_rate: data.cpus[i].refreshRate,
  //         image: data.cpus[i].image,
  //       };
  ...
  try {
    const response = await fetch("/backend/fetchCPUData", {
        method: "POST",
        headers: { "Content-type": "application/json" },
      });
    ...
    for (let i = 0; i < data.cpus.length; i++) {
      let card = {
        product_name: data.cpus[i].productName,
        base_freq: data.cpus[i].baseFreq,
        max_freq: data.cpus[i].maxFreq,
        cores: data.cpus[i].cores,
        threds: data.cpus[i].threds,
        tdp: data.cpus[i].tdp,
        type: data.cpus[i].type,
        image: data.cpus[i].image,
      };
      // console.log(cards.length);
      cards.push(card);
    ...
    const postMethods = () => {
      ...
      cards.map((postData) => {
        ...
        postElement.innerHTML = `
            <img src="${postData.image}" alt="" srcset="" class="card-image">
            <h3 class="card-name">${postData.product_name}</h3>
            <p class="card-text resoluiton">Base Frequency: ${postData.base_freq}</p>
            <p class="card-text resoluiton">Max Frequency: ${postData.max_freq}</p>
            <p class="card-text display-size">Cores: ${postData.cores}</p>
            <p class="card-text panel-size">Threds: ${postData.threds}</p>
            <p class="card-text refresh-rate">Tdp: ${postData.tdp}</p>
            <p class="card-text refresh-rate">Type: ${postData.type}</p>
            `;
        postContainer.appendChild(postElement);
      });
    };
